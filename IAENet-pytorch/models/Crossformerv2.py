...
from layers.SelfAttention_Family import AttentionLayer, FullAttention, TwoStageAttentionLayer, TimeAwareFiLM, FiLM
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if configs.TAFiLM:
            ...
            self.film = TimeAwareFiLM(self.static_numbers, self.dynamic_numbers, hidden_dim=64)  # TimeAwareFiLM
            # Embedding
            self.enc_pos_embedding = nn.Parameter(
                torch.randn(1, self.dynamic_numbers, self.in_seg_num, configs.d_model))
            self.dec_pos_embedding = nn.Parameter(
                torch.randn(1, self.dynamic_numbers, (self.pad_out_len // self.seg_len), configs.d_model))
        else:
            # Embedding
            self.enc_pos_embedding = nn.Parameter(
        ...
        if self.task_name == 'imputation' or self.task_name == 'anomaly_detection':
            ...
            self.projection = nn.Linear(
                self.head_nf * self.dynamic_numbers, configs.num_class)
    ...
    def classification(self, x_enc, x_mark_enc):
        ...
        # # Embedding
        modulated_dynamic = self.film(static, dynamic)

        # embedding
        x_enc, n_vars = self.enc_value_embedding(modulated_dynamic.permute(0, 2, 1))
