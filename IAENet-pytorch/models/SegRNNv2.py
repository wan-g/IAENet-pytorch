...
from layers.SelfAttention_Family import TimeAwareFiLM, FiLM
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if configs.TAFiLM:
            ...
            # Embedding
            self.valueEmbeddingv1 = nn.Sequential(
                nn.Linear(self.dynamic_numbers, self.d_model),
                nn.ReLU()
            )
        else:
            # Embedding
        ...
        if self.task_name == 'classification':
            ...
            self.projection = nn.Linear(
                self.d_model, configs.num_class)
    ...
    def encoder(self, x):
        ...
        x = self.valueEmbeddingv1(x)

        # encoding
        _, hn = self.rnn(x)  # bc,n,d  1,bc,d
        hn = hn.squeeze(0)
        # y = self.predict(hn).view(-1, self.enc_in, self.pred_len)
    ...
    def classification(self, x_enc):
        ...
        # Encoder
        enc_out = self.encoder(modulated_dynamic)
        # Output
