...
class TFTEmbedding(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc=None, x_dec=None, x_mark_dec=None):
        ...
        if x_mark_enc is not None and x_mark_dec is not None:
            ...
            known_input = self.known_embedding(x_mark_enc)
        else:
            known_input = None
...
class TFTDBEmbedding(nn.Module):
    def __init__(self, configs):
        super(TFTDBEmbedding, self).__init__()
        self.pred_len = configs.pred_len
        self.static_pos = datatype_dict[configs.data].static
        self.observed_pos = datatype_dict[configs.data].observed
        self.static_len = len(self.static_pos)
    ...
    def forward(self, x_static, x_dynamic):
        """
               x_static: [B, D_static]
               x_dynamic: [B, T, D_dynamic]
               return:
                   static_input: [B, C_static, d_model]
                   observed_input: [B, T, C_dynamic, d_model]
                   known_input: None  (since we have no timestamp)
        """

        if self.static_len:
            # static_input: [B,C,d_model]
            static_input = torch.stack([embed(x_static[:, i].unsqueeze(1).unsqueeze(2), None).squeeze(1) for i, embed in enumerate(self.static_embedding)], dim=1)
        else:
            static_input = None

        # observed_input: [B,T,C,d_model]
        observed_input = torch.stack([embed(x_dynamic[:, :, i].unsqueeze(-1), None) for i, embed in enumerate(self.observed_embedding)], dim=2)
...
class TemporalFusionDecoder(nn.Module):
    ...
    def forward(self, history_input, future_input, c_c, c_h, c_e):
        ...
        if future_input is not None and future_input.size(1) > 0:
        ...
        # Skip connection
            ...
            temporal_input = history_input
            temporal_features = historical_features
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.embedding = TFTDBEmbedding(configs)
    ...
    def classification(self, x_enc):
        # Normalization from Non-stationary Transformer (与forecast保持一致)
        means = x_enc.mean(1, keepdim=True).detach()
        x_enc = x_enc - means
        stdev = torch.sqrt(torch.var(x_enc, dim=1, keepdim=True, unbiased=False) + 1e-5)
        x_enc /= stdev

        x_static = x_enc[:, 0, -self.static_len:]  # 假设静态特征在最后 D 列
        x_dynamic = x_enc[:, :, :self.observed_len]

        # Data embedding (与forecast相同)
        # static_input: [B,C,d], observed_input:[B,T,C,d], known_input: [B,T,C,d]
        static_input, observed_input, known_input = self.embedding(x_static, x_dynamic)

        # Static context (与forecast相同)
        # c_s,...,c_e: [B,d]
        c_s, c_c, c_h, c_e = self.static_encoder(static_input)

        # Temporal input Selection (类似forecast但只处理历史输入)
        history_input = observed_input
        history_input = self.history_vsn(history_input, c_s)  # [B,T,d]

        # 创建虚拟未来输入
        future_input = torch.zeros(
            (x_enc.size(0), self.pred_len, history_input.size(-1)),
            device=history_input.device
        )  # [B,pred_len,d]
        historical_features = self.temporal_fusion_decoder(history_input, future_input, c_c=c_c, c_h=c_h, c_e=c_e)  # [B, T, d]

        # 分类任务特定处理：
        # 1. 取最后一个时间步的特征作为分类依据
        last_features = historical_features[:, -1, :]  # [B, d]

        # # 2. 可选：添加静态特征增强 (类似forecast中的static enrichment)
        # if c_e is not None:
        #     last_features = last_features + c_e  # [B, d]

        # 3. 通过分类头部
        logits = self.classification_head(last_features)  # [B, num_class]
