...
class moving_avg(nn.Module):
    ...
    def forward(self, x):
        ...
        front = x[:, 0:1, :].repeat(1, (self.kernel_size - 1) // 2, 1)  # 复制序列第一个值，填充到序列开头
        end = x[:, -1:, :].repeat(1, (self.kernel_size - 1) // 2, 1)   # 复制序列最后一个值，填充到序列末尾，保证序列长度一致
        x = torch.cat([front, x, end], dim=1)
        x = self.avg(x.permute(0, 2, 1))  # 平均池化进行移动平均来平滑数据
        x = x.permute(0, 2, 1)
        return x
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.mask_matrix = nn.Parameter(torch.ones(int(self.seq_len / 2) + 1, self.channels))  # (T/2+1, C)
    ...
    def forward(self, x, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        x_fft = torch.fft.rfft(x, dim=1)   # domain conversion (64, 8, 20)
        x_inverse_fft = torch.flip(x_fft, dims=[1])  # flip the spectrum (64, 8, 20)
        x_inverse_fft = x_inverse_fft * self.mask_matrix  # (64, 8, 20)
        x_amplifier_fft = x_fft + x_inverse_fft   # (64, 8, 20)
        x_amplifier = torch.fft.irfft(x_amplifier_fft, dim=1)   # 偶数输入：(2 * (n - 1)) (64, 14, 20)

        # SCI block (可以取消使用)
        if self.SCI:
            x = x_amplifier
            # extract common pattern
            common_pattern = self.extract_common_pattern(x)  # (B,T,1)
            common_pattern = self.model_common_pattern(common_pattern.permute(0, 2, 1)).permute(0, 2, 1)  # (B,T,1)
            # model specific pattern
            specififc_pattern = x - common_pattern.repeat(1, 1, C)  # common_pattern.repeat(1, 1, C) -> (B,T,C)
        ...
        seasonal, trend = self.decompsition(x_amplifier)   # (64,14,20)
